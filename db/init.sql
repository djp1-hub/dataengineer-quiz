...
29	Как можно управлять версионностью потоков данных в Apache NiFi?	1. Использовать NiFi Version Control,2. Использовать Git,3. Использовать NiFi Registry,4. Использовать Control Center	3. Использовать NiFi Registry	3	NiFi
30	Что означает концепция "CQRS"?	1. Разделение между запросами и обновлениями данных,2. Объединение всех операций в один процесс,3. Оптимизация запросов в реальном времени,4. Поддержка многопользовательского доступа	1. Разделение между запросами и обновлениями данных	4	Архитектура
31	Что такое XCom в Apache Airflow?	1. Система обмена сообщениями между тасками,2. Компонент для мониторинга,3. Контрольная точка DAG,4. Функция для создания нового таска	1. Система обмена сообщениями между тасками	3	Airflow
32	Что такое DBT (Data Build Tool)?	1. Инструмент для создания ETL-процессов,2. Инструмент для написания и тестирования SQL-трансформаций,3. Инструмент для хранения данных,4. Инструмент для анализа данных	2. Инструмент для написания и тестирования SQL-трансформаций	3	DBT
33	Что такое транзакционный изоляционный уровень SERIALIZABLE?	1. Позволяет многим транзакциям выполняться параллельно,2. Обеспечивает полную изоляцию транзакций,3. Предотвращает блокировки таблиц,4. Ограничивает доступ к отдельным строкам	2. Обеспечивает полную изоляцию транзакций	3	Postgres
34	Какой архитектурный стиль используется для взаимодействия между распределенными системами с использованием HTTP?	1. SOAP,2. CORBA,3. REST,4. GraphQL	3. REST	2	Архитектура
